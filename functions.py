...
"""Class to be called in MainWindow to setup the port connection"""
...
def update_connection_status(ui, status: str):
    if "Successfully" in status:
        ...
        QtWidgets.QMessageBox.critical(ui, 'Port Error!', 'No port assigned!')
...
def Get_syringe_dict():
    ...
    for syrm_info in syringe_list.values():
        for description, size in syrm_info.items():
            syringe_dic[syrm_info['description']] = list(syrm_info['size'].values())
    return syringe_dic


"""Validity of flow rate parameters, deprecate, using QtGui.QIntValidator() instead"""
...
def on_button_clicked(button, tab, run_params_dict):
    tabs_name = ['Infusion', 'Withdraw', 'Param1', 'Param2']
    if button.property('value') == 'irun':
        tab.setTabText(0, tabs_name[0])
        tab.setTabText(1, tabs_name[3])
        tab.setTabEnabled(0, True)
        tab.setTabEnabled(1, False)
        run_params_dict['Run Mode'] = 'irun'
    elif button.property('value') == 'wrun':
        tab.setTabText(0, tabs_name[1])
        tab.setTabText(1, tabs_name[3])
        tab.setTabEnabled(0, True)
        tab.setTabEnabled(1, False)
        run_params_dict['Run Mode'] = 'wrun'
    elif button.property('value') == 'irun\nwrun':
        tab.setTabText(0, tabs_name[0])
...
def init_combox_syrSize(ui):
    ...
    ui.comboBox_syrManu.currentTextChanged.connect(
        lambda: update_combox_syrSize(ui.comboBox_syrManu.currentText(), ui))


# Update function of comboBox 2
def update_combox_syrSize(text, ui):
    selected_key = text
    selected_values = Get_syringe_dict().get(selected_key, [])
    ui.comboBox_syrSize.clear()
    ui.comboBox_syrSize.addItems(selected_values)


"""Logic of switch between syringe selection (comboBox-Group) and user defined syringe parameters"""
def update_combox_syr_enabled(ui):
...
def add_to_list(item_text, item_icon, parameters_dict, ui_list_widget):
    ...
    if item_short in parameters_dict.keys():
        suffix = 1
        while f"{item_short}_{suffix}" in parameters_dict.keys():
            suffix += 1
        item_short = f"{item_short}_{suffix}"
        parameters_dict[item_short] = ''
    else:
        parameters_dict[item_short] = ''
        # print(item_short)
    ui_list_widget.addItem(item)
...
def delete_selected_item(list_widget, parameters_dict, del_btn):
    ...
    #         update_item_numbers(list_widget, parameters_dict, del_btn)
    #         # print('Selected item:', item_lst.text())
    #         update_parameters_dict(list_widget, parameters_dict, list_widget.currentItem(), key)
    selected_item = list_widget.currentItem()
    if selected_item:
        row = list_widget.row(selected_item)
        key = selected_item.text().split(".")[1].strip()
        update_item_numbers(list_widget, parameters_dict, del_btn)
        update_parameters_dict(list_widget, parameters_dict, selected_item, key)
...
def update_item_numbers(list_widget, parameters_dict, del_btn):
    for i in range(list_widget.count()):
        item = list_widget.item(i)
        item.setText(f"{i + 1}. {item.text()[3:]}")
    del_btn.disconnect()
    del_btn.clicked.connect(lambda: delete_selected_item(list_widget, parameters_dict, del_btn))


# update the steps dictionary after deletion of steps
def update_parameters_dict(list_widget, parameters_dict, item, key_to_remove=None):
    # 获取当前选中item在所有重复items列表中的索引
    ...
    if key_to_remove:
        if len_items_same == 1:
            parameters_dict.pop(key_to_remove, None)
        elif len_items_same > 1 and selected_item_index == 0:
            parameters_dict.pop(key_to_remove, None)
        else:
            parameters_dict.pop("{}_{}".format(key_to_remove, selected_item_index), None)
    else:
        pass
        # for i in range(list_widget.count()):
        #     item = list_widget.item(i)
...
# def edit_item_parameter(list_widget, parameters_dict, item):
#     # print('edit_item_parameter called')
#     current_row = item.listWidget().row(item)
#     # 获取当前item对应的key
#     item_text = item.text().split(".")[1].strip()
#     # 如果当前key已经存在，加上一个后缀"_数字"
#     if item_text in parameters_dict.keys():
#         suffix = 1
#         while f"{item_text}_{suffix}" in parameters_dict.keys():
#             suffix += 1
#         item_text = f"{item_text}_{suffix}"
#     # 设置默认值为item的参数
#     default_value = parameters_dict.get(item.text().split(".")[1].strip(), "")
...
#         parameters_dict[item_text] = item_parameter
...
def edit_item_parameter(list_widget, ui_step_guide, parameters_dict, item):
    # print(parameters_dict)
    item_text = item.text().split(".")[1].strip().split()[0]
    ...
    # 所选择的元素没有重复项
    if len_items_same == 1:
        default_value = parameters_dict.get(item_text, '')
        ui_step_guide.lineEdit.setText(default_value if default_value is not None else '')
    # 所选择的元素有重复项，例如：index为2的Constant --> default_value = parameters_dict["{}_{}".format(Constant, 2))]
    # index为0时：
    elif len_items_same > 1 and current_index == 0:
        default_value = parameters_dict.get(item_text, '')
        ui_step_guide.lineEdit.setText(default_value if default_value is not None else '')
    else:
        default_value = parameters_dict.get("{}_{}".format(item_text, str(current_index)), '')
        ui_step_guide.lineEdit.setText(default_value if default_value is not None else '')
    ...
    if ui_step_guide.buttonBox.accepted:
        ...
        # index >= 1时：
            ...
            parameters_dict["{}_{}".format(item_text, str(current_index))] = item_parameter
        # print(parameters_dict)
...
def print_parameters_dict(parameters_dict):
    # 按照字典值的顺序排列字典
    sorted_dict = {k: v for k, v in sorted(parameters_dict.items(), key=lambda item: list(parameters_dict.keys()).index(item[0]))}
    new_dict = {}
    counter_dict = {}
...
def import_user_defined_methods(list_widget, parameters_dict):
    # 打开目录选择文件
    ...
    if file_path:
        with open(file_path, "r") as f:
            ...
            if isinstance(imported_dict, dict):
                imported_dict = print_parameters_dict(imported_dict)
                for key, value in imported_dict.items():
                    # 判断是否有重复的键，如果有加上一个后缀
                    # new_key = key
                    # suffix = 1
                    # while new_key in parameters_dict.keys():
                    #     new_key = f"{key}_{suffix}"
                    #     suffix += 1
                    parameters_dict[key] = value
                # 更新listWidget中的显示
                update_list_widget(list_widget, parameters_dict)
                return parameters_dict
            else:
                QtWidgets.QMessageBox.information(list_widget, 'Invalid import', 'Data tried to be imported is not a '
                                                                                 'dictionary type data!')


# Update the list shown in MainWindow (QListWidget for methods)
def update_list_widget(list_widget, parameters_dict):
    # print('update_list_widget called!')
    # 清空listWidget
    list_widget.model().removeRows(0, list_widget.model().rowCount())
    # 添加每一个键值对到listWidget中
    for i, (key, value) in enumerate(parameters_dict.items()):
        # 根据key选择icon
...
def export_user_defined_methods(parameters_dict):
    # 获取保存路径
    ...
    if parameters_dict:
        # 转换为标准排序格式：
        print_parameters_dict(parameters_dict)
        # 导出parameters_dict为json文件
